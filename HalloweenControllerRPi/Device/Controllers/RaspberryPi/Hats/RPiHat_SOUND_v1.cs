    ...
    public class RPiHat_SOUND_v1 : RPiHat
    {
        ...
        private void Chan_ChannelUpdated(object sender, SoundChannelEventArgs e)
        {
            ...
            switch (e.NewState)
            {
                ...
                case SoundState.Status:
                    soundDrivers[(int)sndChan.Index].Status();
                    break;
        ...
        private void InitialiseSoundDrivers()
        {
            ...
            for (int i = 0; i < busDevice.NumberOfUARTChannels; i++)
            {
                ...
                sndDrv.StatusChanged += SndDrv_StatusChanged;
                soundDrivers.Add(sndDrv);
            }
        }

        private void SndDrv_StatusChanged(object sender, SoundProviderStatusEventArgs e)
        {
            uint index = (uint)soundDrivers.IndexOf((sender as ISoundProvider));

            HostController.OnChannelNotification(Channels[(int)index], new CommandEventArgs('S', 'C', index, (uint)e.Status));
        }
