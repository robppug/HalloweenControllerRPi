...
namespace HalloweenControllerRPi
{
   ...
   {
      #region "XML Loading"
      private void button_LoadCfg_Click(object sender, EventArgs e)
      {
         try
         {
            using (OpenFileDialog fileDialog = new OpenFileDialog())
            {
               fileDialog.Filter = "Sequence File|*.sqn";
               fileDialog.Title = "Load sequence file";
               fileDialog.ValidateNames = false;

               if (fileDialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
               {
                  this.loadedFileName = fileDialog.FileName;

                  StreamReader loadfile = new StreamReader(this.loadedFileName);
                  XmlReader xmlReader = XmlReader.Create(loadfile);

                  XDocument xDoc = XDocument.Load(XmlReader.Create(new StreamReader(this.loadedFileName)));
         ...
         catch (IOException exp) { MessageBox.Show(exp.ToString()); }
      }
