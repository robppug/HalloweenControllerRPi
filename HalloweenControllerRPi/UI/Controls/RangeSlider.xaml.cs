...
namespace HalloweenControllerRPi.Controls
{
    public sealed partial class RangeSlider : UserControl
    {
        ...
        private async void PopupValueEntry(Thumb thumb)
        {
            TextBox textBox;
            ...
            ContentDialog contentDialog = new ContentDialog()
            {
                ...
                Content = new TextBox()
                {
                    Text = (thumb == MinThumb ? RangeMin.ToString() : RangeMax.ToString()),
                    TextWrapping = TextWrapping.Wrap
                },
                DefaultButton = ContentDialogButton.Close,
            };

            textBox = (contentDialog.Content as TextBox);
            textBox.SelectionStart = textBox.Text.Length;

            textBox.KeyDown += (sndr, ev) =>
            {
            ...
            textBox.TextChanged += (sndr, ev) =>
            {
                double enteredValue;

                if (double.TryParse(textBox.Text, out enteredValue))
                {
                    if (enteredValue > this.Maximum)
                    {
                        enteredValue = this.Maximum;
                    }
                    else if (enteredValue < this.Minimum)
                    {
                        enteredValue = this.Minimum;
                    }
                    if (thumb == MinThumb)
                    {
                        RangeMin = enteredValue;
                    }
                    else
                    {
                        RangeMax = enteredValue;
                    }
        ...
        {
            PopupValueEntry(sender as Thumb);
        }
    }
}
